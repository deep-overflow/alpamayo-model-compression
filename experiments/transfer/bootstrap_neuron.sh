...
No .venv yet. Build it with the companion script -- it encodes two site workarounds
that are easy to trip over:

  /bin/bash REPO_PLACEHOLDER/experiments/transfer/build_venv_neuron.sh

  * `module load python/3.12.4` is DEPRECATED and redirects to 3.14.2, which violates
    pyproject's "==3.12.*" pin and has no torch 2.8 wheels. The script takes its
    interpreter from uv's managed CPython instead.
  * flash-attn is required (config.json sets attn_implementation=flash_attention_2) and
    uv.lock only has an sdist, so `uv sync` would compile it. The script installs the
    prebuilt cu12torch2.8/cxx11abiTRUE/cp312 wheel instead -- no toolchain, no wait.

Verified result on NEURON: python 3.12.14, torch 2.8.0+cu128 (cuda 12.8),
transformers 4.57.1, peft 0.20.0, flash_attn 2.8.3, pandas 3.0.1 -- the source box's
versions exactly.
ENVNOTE
fi
