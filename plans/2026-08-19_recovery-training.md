...
## 부록 (2026-08-20): u70 확장

u55 회복이 세 게이트를 모두 통과했으므로 (test @6 1.008, degen 0.000, 폐루프 +0.0736),
같은 레시피를 더 깊은 프루닝에 적용해 회복 한계를 탐색한다.

- 대상: `dual_u70_v2` (uniform 0.70, 신규 빌드 — `_u<pct>_v2` 계열 dispatch).
- 변경점은 둘: 프루닝 비율(0.55→0.70)과 **학습 길이 600→1,200 step** (~7.8 epoch;
  u55에서 step 600까지 val이 계속 내려갔고 손상이 더 크므로 — patience 3 조기 종료가
  과적합을 방어). 데이터·probe·KI 구조·lr(1.4e-4)·global batch(16)는 동일.
- 게이트 (u55와 동형): G1' = test @6에서 zero-shot 대비 개선 CI<0 및 u70→u40 gap의
  80% 회복 (수치 목표선은 zero-shot 앵커 측정 후 확정), G2' degen ≤ 0.05,
  G3' 폐루프 Δscore CI 하한 > −0.080.
