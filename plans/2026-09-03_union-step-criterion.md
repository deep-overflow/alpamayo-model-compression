...
## 결과 2차: test500 + OOD-val (2026-09-04, `maxstep11`만)

| 세트 | `maxstep11` minADE@6 | `dualfix` | `maxstep11 - dualfix` med [CI] | p |
|---|---|---|---|---|
| val500 | 0.8915 (0.5991) | 0.9236 (0.6398) | -0.0033 [-0.0162, +0.0042] | 0.19 |
| test500 | 0.9351 (0.6339) | 0.9649 (0.6658) | -0.0115 [-0.0316, +0.0038] | 0.062 |
| OOD-val | 1.1039 (0.7113) | 1.1303 (0.6768) | +0.0011 [-0.0192, +0.0180] | 0.90 |

**B1 PASS (세 세트 전부)** — 최대 |median| 0.0115로 게이트(0.05)의 1/4. 스텝 축은 합집합
연산 아래에서 무해하다. `maxstep11 - znorm11`은 세 세트 모두 크게 유의
(-0.150 / -0.230 / -0.130, 전부 CI가 0 제외), 즉 같은 11개 손실이라도 결합 방식만으로
이만큼 갈린다.

**꼬리 효과가 세 세트에서 재현된다** (`dualfix` minADE 상위 10% 기준):

| 세트 | 쉬운 90% mean | 어려운 10% mean | 어려운 10% median |
|---|---|---|---|
| val500 | +0.0220 | **-0.5186** | -0.1245 |
| test500 | +0.0074 | **-0.3646** | -0.2050 |
| OOD-val | +0.0415 | **-0.6164** | -0.3696 |

전형적 클립에서는 차이가 없고 어려운 구간에서만 이득이 난다. 세 세트 독립 재현이므로
val500 단독일 때보다 근거가 강하다. 다만 전체 페어드는 여전히 유의하지 않으므로
"평균적으로 더 좋다"가 아니라 **"꼬리에서만 다르다"** 가 정확한 진술이다.

CoC 퇴화율: val 2.2% / test 2.6% / OOD 3.8% (`dualfix`는 1.8 / 4.0 / 3.8%). B3 PASS.

**다음으로 갈 만한 곳**: 꼬리 이득이 실재한다면 그것이 드러나는 곳은 개루프 평균이 아니라
폐루프다. `maxstep11`을 alpasim hard100(어려운 100씬) 같은 스위트에 올리는 것이 자연스러운
후속이다. 남은 교란(`mean of 11 rank_norm` 대조군)은 여전히 미실행.
