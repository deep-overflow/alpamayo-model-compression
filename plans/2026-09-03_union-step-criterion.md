...
## 정정 (2026-09-04): 꼬리 이득은 선택 편향이었다

위 "꼬리 효과가 세 세트에서 재현된다" 표는 꼬리를 **`dualfix`의 자기 점수**로 정의했다.
한 arm의 자기 점수로 극단을 고르면 그 극단성의 일부가 노이즈이고 비교 arm은 그것을 공유하지
않으므로, 비교 arm이 자동으로 좋아 보인다(평균 회귀). 네 가지 선택 규칙으로 다시 재면:

| 꼬리 선택 기준 | val500 | test500 | OOD-val |
|---|---|---|---|
| `dualfix` 점수 (초판) | -0.5186* | -0.3646* | -0.6164* |
| `maxstep11` 점수 (반대) | **+0.0646** | **+0.0846** | **+0.1373** |
| 두 arm 평균 (대칭) | -0.1186 (p=0.50) | -0.1149 (p=0.13) | -0.4051 (p=0.086) |
| baseline (중립) | +0.0453 (p=0.20) | -0.1843 (p=0.14) | +0.0779 (p=0.75) |

선택 기준을 반대 arm으로 옮기면 세 세트 모두 **부호가 뒤집힌다**. 편향 없는 두 규칙에서는
유의한 효과가 없다. "세 세트 재현"도 근거가 못 된다 — 같은 편향 추정량을 세 번 적용하면
편향이 재현될 뿐이다.

**정정된 결론**: `maxstep11`은 `dualfix`와 전체에서도 꼬리에서도 구분되지 않는다. B1 PASS는
"동급"을 뜻하고 그 이상이 아니다. 이에 따라 "어려운 씬 폐루프로 확인하자"는 후속 제안도 철회한다.
상호작용 결론(단독 +0.006 / -0.003, 둘 다 +0.173)은 선택 편향과 무관하므로 그대로 유효하다.

**교훈**: 부분집합을 한 arm의 자기 지표로 정의했으면, 믿기 전에 반드시 (a) 반대 arm으로
재선택하고 (b) 제3의 변수로도 재선택해 볼 것.
