...
- **혼합 6점** (저장된 스트림별 H의 선형 결합으로 사후 생성):

  | 이름 | D 가중 | 정체 |
  |---|---|---|
  | `VT` | 0× | **현행 Tyr의 H 그 자체** (prefill only) |
  | `nat` | 1× (자연 0.5%) | **논문 레시피를 문자 그대로** 적용한 concat |
  | `d10` / `d100` / `d1000` | 10/100/1000× | 토큰수 정규화 이식 (이번에 검증할 것) |
  | `Donly` | ∞ | 텍스트-only 코너, 조건수로 이미 탈락한 지점 (진단용) |

  따라서 **`nat` − `VT`가 "논문을 문자 그대로 쓰면 얼마나 얻나"**를, `d10` − `VT`가 "정규화
  이식이 얼마나 얻나"를 잰다. (원안이 `nat`을 현행 Tyr로 적은 것은 오기 — prefill-only는 `VT`.)
...
### 2026-08-25 — G0는 damp 1e-2에서 well-posed하지 않다

`VT` 혼합(= Tyr의 prefill-only H)으로 layer 0을 풀어 `slim_tyr_uniform_u40_recon`과 비교한
결과 **kept-set은 o_proj 완전 일치 / MLP 99.0% 일치인데 가중치는 크게 다르다**:

| | ‖ours‖ | ‖ref‖ | ours vs dense | ref vs dense | ours vs ref |
|---|---:|---:|---:|---:|---:|
| o_proj | 87.1 | 104.2 | 1.174 | 1.566 | **0.602** |
| down_proj | 119.6 | 119.6 | 0.502 | 0.501 | **1.297** |

두 해는 **dense로부터 사실상 같은 거리**(MLP 0.502 vs 0.501)에 같은 노름으로 놓여 있다. 즉
서로 다른 코드가 아니라, **`inv(H_kk)`가 비유일**한 것이다 — `outputs/tyr_hdiag.json`이
cond ≈ 2e34, damp 1e-2에서 kept-weight `rel_change` 3.55라고 이미 기록해 두었다. fp32
누적 순서 차이(우리는 V/T를 따로 쌓아 합치고 Tyr은 한 버퍼에 쌓는다)만으로도 해가 갈린다.

**따라서 G0을 가중치 동일성으로 판정하는 것은 잘못된 설계다.** 대신:

- **G0a (조건화된 재현)** — damp **1.0**에서 `slim_tyr_uniform_u40_d1`과 비교. 이 영역은
  `rel_change` 0.12로 well-conditioned하므로, 여기서 일치하면 **산술 경로가 Tyr과 동일함이
  증명**된다. 이것을 재현 게이트의 본안으로 삼는다.
- **G0b (결과로 승격)** — damp 1e-2에서의 불일치는 버그가 아니라 결과다:
  **현행 Tyr 재구성은 run-to-run 수치적으로 재현되지 않는다.** 리포트에 명시한다.

## 결과 (2026-08-25)

`racfit_v1_l{00,09,18,27}` 36레이어, `calib_100` 100클립, 2-fold held-out, K=4, damp 1e-2.
리포트 `reports/evaluation/2026-08-25_cot-reconstruction.html`.

| 게이트 | 결과 | 판정 |
|---|---|---|
| **G0a** 재현 | damp 1.0에서 목적값 0.168015 vs 0.168016 (4e−6), down_proj 7e−5 | **PASS** |
| **G0b** | damp 1e−2는 목적값 2.91% 차이, MLP 채널 77/7390 다르게 선택 | 재현 불가(결과로 보고) |
| **G1** 전제 | Spearman 0.257 / 0.223, 상위-512 에너지 중첩 0.447 / **0.117** | **PASS** |
| **G2** 본안 | err_D(nat)−err_D(d10): o +0.0386 [+0.0073,+0.0507] p=0.012 · m +0.0361 [+0.0153,+0.0697] p=2.9e−11 | **PASS** |
| **G2b** | vs `VT`(현행 Tyr): o +0.164 · m +0.185 | PASS |
| **G2c** | 논문 문자 그대로(`nat`)가 o +0.145 / m +0.151 — **전체의 88% / 81%** | PASS |
| **G3** 선택 이동 | 0.849 / 0.893 (바닥 0.860 / 0.782) | FAIL → **효과는 재적합** |
| **G4** 비용 | o +0.0146 [+0.0112,+0.0408] · m +0.0049 [+0.0035,+0.0069] | o FAIL / m PASS |
| **G5** 배분 | r*(0.10) 표준편차 o 0.058 · m **0.287** | **PASS** |

**핵심**: `VT`(= 현행 Tyr H)는 재구성이 decode 경로를 "재구성 안 함"보다 **나쁘게** 만드는
유일한 arm이다 — o +0.0349, m +0.0197 (CI 0 배제). 같은 재구성이 vision(−0.032/−0.028)과
프롬프트 텍스트(−0.092/−0.053)는 개선한다. 자기 CoC를 자연 비중 1.92%로만 넣어도 두 부호가
뒤집힌다. 선택을 `dual`로 고정하고 H만 바꿔도 decode −26%(o) / −35%(m), vision 불변.

**정칙화 대조군** (`racfit_ctrl`, 계획에 없던 추가 대조): 같은 배율로 decode 대신 프롬프트
텍스트를 올리면(`t4`/`t10`) decode 오차가 ±0.006 안에서 안 움직인다. 배율 맞춘 직접 비교
err_D(t10) − err_D(d10) = **+0.164 (o) / +0.180 (m)**, p<1e−5. `t10`은 자기 스트림은
제대로 개선하므로(err_T +0.073 / +0.017) 고장난 것이 아니다. **효과는 decode 특이적이며
조건수 개선이 아니다.** 부수적으로, 배포된 `VT` 대비로는 `d10`이 vision에서도 비용이 없다
(err_V 차 −0.011 / −0.006) — G4의 비용은 `nat` 기준일 때만 존재한다.

**레이어별 한계**: ε=0.10에서 MLP 레이어 0–5가 70–83%, 레이어 6–33은 5–13%(34–35에서 재상승).
o_proj는 평평·낮고(평균 0.094) 레이어 7이 가장 민감(r*≈0). u40 예산을 전 레이어 동일 오차
(rel_err 0.2279)로 재배분하면 예산 오차 0.000%, Q head 평균 0.239(uniform 0.3986보다 40%
적게)·MLP 평균 0.436. `outputs/racfit_v1/alloc.npz`에 `run_grid.allocations()` 형식으로 저장.
Lagrangian water-filling은 오차 곡선이 오목해 bang-bang으로 퇴화하므로 등오차가 주 해석.

## 상태

**완료** 2026-08-25. 후속(체크포인트 빌드·평가)은 별도 승인 대상.
