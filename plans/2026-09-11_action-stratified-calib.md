...
### 4. 중요도 9/9 완료 — 선택 수준의 사실 (2026-09-11 22:45 KST)

cvlab20에서 rd_c/se_a/se_b/se_c 4개 동시(22:18–22:30), su_a/b/c 3개 동시(22:30–22:42), 전부
exit 0, `n_clips=100`. slim 9개는 빌드당 ~30초. 회수한 9개 + `importance_v2`(calib_100) +
`importance_nt500_a..e`로 `dual_u40_v2` 유지집합을 만들어 중첩을 재면(분석기 `kept_overlap`):

| 쌍 | Q heads | MLP |
|---|---:|---:|
| 규칙 내 rd / se / su / nt | 90.6 / 90.9 / 90.7 / 91.1% | 85.4 / 85.3 / 85.8 / 85.7% |
| 규칙 간 rd–se / rd–su / se–su | 90.7 / 90.3 / 90.6% | 85.3 / 85.1 / 85.3% |
| 각 규칙 – nt (15쌍씩) | 90.5–91.2% | 85.4–85.5% |
| 각 arm – calib_100 | 89.5–91.4% | 83.9–84.8% |

**추출 규칙은 선택을 n=100 잡음 바닥 이상으로 전혀 움직이지 않는다** — cruise 56%→20%인 su와
rd의 쌍 중첩(90.3%)이 rd 안의 쌍(90.6%)과 같다. `2026-09-03` §7("시나리오 재가중은 선택을
무작위 이상으로 못 바꾼다")의 전향적 재현이고, `calibration-size`의 n=100 바닥(9.0%/14.6%
변위)과 크기가 같다. 게이트가 아니라 해석의 기준선: 이 레포에서 중첩은 성능을 예측하지 못하므로
(`dual_st2000` 92.1% → +0.1105 vs `maxstep11` 92.5% → n.s.) 판정은 test500이 한다. 다만
결과가 어느 쪽이든 "선택은 같은데 성능이 다르다/같다"로 읽어야 한다.

함정: 회수 스크립트가 su의 10클립 부분 체크포인트를 가져온 채 중첩을 재면 81%/77%가 나온다 —
분석기는 `n_clips==100`인 런만 쓰도록 고쳤다.

### 5. 결과 (2026-09-12 02:00 KST 체인 완료, 9 arm × test500)

`outputs/strat_calib/{metrics.json,summary.txt,plots/}`. 전부 cvlab20(Ada), `baseline_ada_ps_test`와
페어링. arm 단위 minADE@6 — rd 1.379/1.180/1.025 (평균 1.195, SD 0.177), se 1.008/1.136/0.981
(**1.042, SD 0.083**), su 1.184/1.051/1.181 (1.139, SD 0.076); nt 5블록 1.384 (SD 0.354),
calib_100 0.950.

| 대조 (클립별 3-seed 평균) | 전체 | cruise | stop | accel | turn |
|---|---|---|---|---|---|
| rd − nt | −0.190 [−0.234,−0.147] p=3e-23 | −0.365* | −0.022 | −0.026 | +0.120 (p=0.12) |
| se − rd | −0.153 [−0.190,−0.117] p=2e-21 | −0.180* | −0.100* | −0.088* | −0.161* |
| su − rd | −0.056 [−0.096,−0.018], med −0.012 p=0.13 | −0.015 | +0.034 | −0.069 | −0.253* p=0.0016 |
| su − se | +0.097 [+0.058,+0.136] p=2e-10 | +0.165* | +0.134 (p=0.07) | +0.019 | −0.092 |

**게이트**: G1 "해롭다"(랜덤 > 그리디 순차, 전부 cruise; 단 매칭과 순차 배제는 미분리, calib_100은
여전히 최고). **G2 기각**(1/3 — turn만 −0.25). G3 랜덤 대비 대가 없음(se 대비 cruise +0.165).
**G4 기각, 반대 방향** — se가 rd보다 전 버킷 유의하게 좋고 분산 절반. G5 SD: nt 0.35 ≫ rd 0.18 >
층화 0.08. 선택 중첩은 전 규칙 동일(§4)인데 성능은 0.15–0.19 갈림.

**판독(사후)**: action 조성이 품질을 움직이고 목표는 다양성이 아니라 **평가 분포**다. 랜덤은
희귀 action을 가끔 놓치고(rd_a: cruise 65, turn_left 2) 그때 전체가 무너지며, 균등 층화는 turn을
사되 cruise·stop을 잃는다. 사전 등록 결정표엔 "se > rd" 칸이 없었다(H4가 null 예측). **권고:
표준 추출을 `se`로** — 개루프 근거뿐이므로 변경 전 hard100 폐루프 1회(se_c vs rd_c 또는
calib_100). 별건: rd_b CoC 퇴화 6.8%.

**남는 한계**: 3 seed(SD 참고치), test500만, 순차 그리디의 "순서 vs 매칭" 미분리(draw-order 플랜
그대로), su의 turn 이득이 폐루프 lane-keeping과 연결되는지는 미측정.
