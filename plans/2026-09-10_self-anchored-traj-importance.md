...
| **G3** 안정성 (**본 가설**) | calib_100 50:50 split-half kept 겹침이 GT 앵커의 **Q 0.860 / MLP 0.782 이상**일 것. 같은 분할·같은 예산으로 비교 | H1 기각. 멀티모달 분산 논거가 틀린 것 — 그 자체로 보고 가치 있음<br>**→ 실제로는 미해결: 표본 수와 분리하지 못했다. 아래 결과 절 참조** |
...
## 결과 (2026-09-10 ~ 09-11 완료)

계획 대비 실제로 무엇이 나왔는지. **G3는 설계 결함으로 미해결**이고, 그 결함 자체가 발견이다.

| 게이트 | 계획 | 결과 |
|---|---|---|
| **G1** 결합 선택 | A가 median 0.019 이상이면 A | **A 채택.** 100클립에서 A=0.0578(gt 0.1932의 29.9%), 문턱의 3.0배. 이유도 측정됨 — flow 직선성 `‖v−chord‖/‖chord‖`=0.168이라 퇴화 안 함 |
| **G2** no-op 검사 | 재현성 천장(0.997)보다 낮을 것 | **통과.** dual 대비 겹침 Q 0.9327 / MLP 0.9071 |
| **G3** 안정성 | split-half가 GT 앵커 이상 | **미해결 (아래)** |
| **G4** 개루프 | val500 페어드 | **기각.** 세 세트에서 +0.054 / +0.105 / +0.119, 12개 CI 전부 0 배제 |

### G3가 미해결인 이유 — 대조군 설계가 틀렸다

자기 앵커가 split-half에서 +0.0357(Q) / +0.0260(MLP) 앞선 것은 사실이다. 그러나 **표본 수와
분리되지 않았다**: 출하 경로는 각 `t`를 노이즈 하나로만 보는데 tied 자기 앵커는 K=10개 chord로
`t`당 10개를 평균한다.

표본 수를 맞춘 대조군(`gtk`, `--k-draws`)을 만들었으나 **그 시도가 실패했고 그 실패가 별개의
발견이다.** 출하 점수는 `|Σ_s ∂L_s/∂g|`이라 K를 올리면 평균이 되는 게 아니라 **부호 있는 합의
항이 늘어난다**:

| K | `traj_vlm_q` mean | 클립별 분산 | kept set (출하본 대비) | 개루프 |
|---|---:|---:|---:|---|
| 1 | 8.78e-03 | 1.3745 | — | — |
| 3 | 2.20e-02 | 1.3177 | 96.1% | null |
| 5 | 3.83e-02 | — | 96.2% | null |
| 10 | 7.71e-02 (**8.8배**) | 1.3490 | 96.2% | null |

크기만 커지고 분산은 그대로다. `rank_norm`이 스케일 불변이므로 **순위 기반 선택에서는 사실상
스케일 연산**이고, 개루프에서도 재현되는 효과가 없다(세 세트 12개 비교 중 test500의 K=3·K=10만
CI가 0을 배제, 평균으로는 전부 소멸).

올바른 대조군은 `Σ_draws |Σ_steps ∂L/∂g| / K` — draw마다 gate grad를 읽고 0으로 만들어야 한다.
`expert_fm_grads`가 cache leaf grad만 반환하고 gate는 바깥에서 읽는 현 구조로는 안 되어 재작업이
필요하다. **G4가 이미 자기 앵커를 기각했으므로 결론은 바뀌지 않는다.**

### 계획에 없던 arm — best-of-n

G4 실패의 진단("기준이 GT를 전혀 모르게 됐다")에서 나온 절충안. n=6 draw 중 GT 최근접을 타깃으로
쓴다 — on-manifold이면서 GT가 고른 궤적. **셋 중 가장 나빴다**(+0.130 / +0.164 / +0.147).
다만 CoC 퇴화는 고쳤다(5.4% → 1.6%).

해석: 타깃은 on-manifold가 아니라 **전형적**이어야 한다. GT는 데이터의 전형, 무작위 draw는 모델의
전형, best-of-6은 **어느 쪽의 전형도 아닌 순서통계량**이다.

### 계획이 예측하지 못한 것

계획서의 "대가" 항목 — *"기준이 GT를 전혀 모르게 되므로 모델을 옳게 만드는 유닛과 틀리게 만드는
유닛을 구별하지 못한다"* — 은 맞았고 CoC 퇴화 4배가 그 직접 증거다. 그러나 **안정성과 품질이
반대 방향으로 움직인다**는 것은 예상하지 못했다. 추정량을 더 안정적으로 만들었는데 더 나쁜
유닛을 골랐다. 이것이 이 실험의 주 결과이며, 기존의 "kept set 변위 크기는 손상을 예측하지
못한다"와 독립적인 두 번째 사례다 — **선택 통계만으로 기준을 고르는 모든 경로가 막힌다.**

보고서: `reports/evaluation/2026-09-10_selftraj-anchor.html`
...
- ~~G4는 G2·G3를 통과한 뒤에만~~ — G3가 미해결로 끝나 G4를 먼저 돌렸다. 기전 검정을 못 해도 결과 검정은 유효하다
