...
날짜: 2026-09-19. 브랜치: `worktree-fastpath-rounds`. 상태: **완료 (2026-09-19 13:06–14:00 KST 측정, 보고서 `reports/evaluation/2026-09-19_fastpath-single-card-rounds.html`)**.
...
### 1. 실행 (2026-09-19, cvlab21 Ada 4번 한 장)

계획 승인 후 코드 작성 → prefetch 25클립(CPU, 11분, 2.4 GB) → Blackwell 1번에서 2클립 스모크
(em93p75, peak 14.26 GiB 선행과 일치) → 분석기 자체검증(선행 5 run을 가짜 2라운드로: 선행 표 재현,
음성 대조가 선행 em93p75 prefill +4.0–5.0%를 FAIL로 검출). Ada 4–7은 em93p75 hard100 폐루프
체인이 쓰고 있어 **체인이 완전히 끝난 13:06까지 대기**(4·6번이 먼저 비었지만 남은 스택 둘의 CPU
사용 때문에 기다림). 4번 카드 claim, 25 run을 **53분 38초**에 완료(run당 ≈2.1분 — 위 비용 추정
5–6분은 과대), 전부 exit 0, 동거 0건, 재실행 0건. 측정 중 박스 load average 9–36(같은 계정의 다른
작업; GPU 공유는 없음).

### 2. 결과 — `outputs/fastpath_rounds/`

| arm | prefill | dec/tok | denoise | e2e fast | CV % | speedup | 선행 speedup |
|---|---:|---:|---:|---:|---:|---:|---:|
| base | 691.3 ± 3.3 | 27.39 ± 0.02 | 212.3 ± 0.3 | 1341.8 ± 3.4 | 0.26 | 1.000× | 1.000× |
| dual | 515.6 ± 2.2 | 19.53 ± 0.02 | 204.3 ± 0.2 | 1032.4 ± 2.3 | 0.22 | 1.300× | 1.313× |
| em75 | 514.4 ± 1.9 | 19.52 ± 0.03 | 160.3 ± 0.2 | 987.0 ± 2.3 | 0.23 | 1.359× | 1.374× |
| em87p5 | 513.4 ± 2.6 | 19.52 ± 0.04 | 153.8 ± 0.2 | 979.6 ± 3.4 | 0.34 | 1.370× | 1.391× |
| em93p75 | 514.1 ± 1.7 | 19.54 ± 0.03 | 150.9 ± 0.2 | **977.8 ± 2.0** | 0.21 | **1.372×** | 1.362× |

(ms, run 값 5개의 평균 ± SD; e2e는 16토큰 정규화.)

- **G0 PASS**: 25/25, 전부 `gpu_index` 4, live 24클립·라운드 간 동일, 동거 0건, peak 범위 0.000 GiB,
  `fast_steps` 라운드 간 동일.
- **G1 MARGINAL (11/12 PASS)**: 최대 오프셋 0.37%(em87p5/dual prefill). 미달 한 쌍은 em75/dual
  dec/tok 0.9992 [0.9983, 0.9999] — 오프셋 0.08%인데 CI가 1을 0.0001 차이로 배제. 같은 VLM 네 arm의
  prefill 폭은 선행 25.3 ms(5.0%) → 2.2 ms(0.4%). **H1 채택.**
- **G2 PASS**: denoise em75/dual 0.7850 [0.7845, 0.7856], em87p5/em75 0.9595 [0.9583, 0.9608],
  em93p75/em87p5 0.9815 [0.9793, 0.9831]. **H2 채택.**
- **G3**: e2e가 단조로 돌아옴(1032.4 → 987.0 → 979.6 → 977.8). em93p75/base 0.7291
  [0.7269, 0.7327]; em93p75/em87p5 **0.9983 [0.9941, 1.0027] 구분 불가** — denoise 차 2.9 ms가
  prefill의 run 간 SD(1.7–2.6 ms)와 같은 크기.
- **G4 PASS**: e2e CV 0.21–0.34%. 같은 run의 stock(eager) 경로는 CV 1.1–3.4%(약 10배).

**판독**: 선행 표의 em 사다리 비단조는 arm마다 다른 카드에서 돌린 run 단위 오프셋이었다. 헤드라인은
거의 그대로(1.36× → 1.37×, stock 무압축 대비 1.56× → 1.55×)이고, 바뀌는 것은 사다리 내부 순서와
dual·em75·em87p5의 1–2% 과대평가다. G1은 측정이 정밀해서 떨어진 게이트 — 다음부터는 "CI가 1을
포함"이 아니라 동등성 임계로 등록할 것.
