...
보고서 `reports/evaluation/2026-08-30_axis-taylor-comparability.html`,
분석 `outputs/{axis_taylor_analysis,vlm_axis_analysis,vlm_axis_check}/`.

### Stage 1 — 저울은 맞다 (expert probe, 16클립 × 1,265 probe)

| probe | n | 기울기 β [95% CI] | r | 부호 일치 |
|---|---:|---|---:|---:|
| Q head 1개 | 9,216 | **+0.985 [+0.903, +1.046]** | 0.918 | 85% |
| MLP channel 1개 | 3,456 | **+0.924 [+0.836, +1.089]** | 0.329 | 34% |
| MLP 85채널 블록(파라미터 매칭) | 4,608 | +0.854 [+0.318, +1.224] | 0.722 | 43% |
| 층별 arm 절단 · Q | 1,152 | +1.099 [+1.027, +1.147] | 0.972 | 89% |
| 층별 arm 절단 · MLP | 1,728 | +0.114 | 0.049 | 3% |

**S1 채택**: β_q / β_mlp = **1.066 [0.890, 1.193]** — 사전 등록 구간 [0.5, 2] 안.
§2의 축 간 raw 배수를 보정 없이 읽어도 된다. S1b(파라미터 매칭) 1.154 [0.772, 3.069].
**S0**: 게이트 경로(float32)와 마스크 경로의 손실 차 median 1.18e-4 — ΔL의 두 항이 모두
마스크 경로라 상쇄된다. 사전 등록한 상대 1e-3 기준은 손실 자체가 ~1e-3인 클립에서 의미를
잃도록 잘못 지정됐다(양쪽 수치를 보고서에 남김); 실증 검증은 β_q ≈ 1이다.

**S2 — 진짜 발견**: 전 층 동시 절단 ÷ 층별 절단 36개의 합

| 절단 | 예측 ΔL | 실측 ΔL | 실측/예측 | 전층/합 | ΔminADE@6 |
|---|---|---|---:|---:|---|
| Q 4/16 | +7.46e-3 | +1.94e-2 | +2.59 | **+1.71** | +0.0563 |
| Q 8/16 | +1.27e-2 | +7.16e-2 | +5.64 | **+2.52** | +0.2701 |
| MLP 341ch | −1.94e-6 | +1.26e-6 | −0.65 | +0.00 | −0.0004 |
| MLP 2064ch | −2.07e-5 | −1.07e-6 | +0.05 | −0.00 | −0.0003 |
| MLP 4128ch | −5.76e-5 | +6.23e-6 | −0.11 | +0.01 | +0.0001 |

Q축에만 **층 간 초가산성**이 있다(1.7–2.5배). 이것이 expert 보고서의 "헤드 2배 → 비용 5배"의
기전이고, 동시에 1차 Taylor가 원리적으로 못 담는 항이다. **S4**: `Σ_s|·|`로 바꾸면 기울기가
0.24/0.34로 떨어져 3–4배 과대예측 — 현행 `|Σ_s|`가 맞는 집계다.

### Stage 2b — VLM 축 분해 (Blackwell, `baseline_bw_ps_*` 대비, paired 중앙값 [95% CI])

| arm | 제거 | val500 | test500 | OOD-val 262 |
|---|---:|---|---|---|
| vq (Q 13/32) | 490.7M | **+0.0340 [+0.0179, +0.0536]\*** | **+0.0384 [+0.0218, +0.0672]\*** | **+0.0406 [+0.0032, +0.0610]\*** |
| vm_pm (MLP 1109ch) | 490.6M | −0.0006 [−0.0025, +0.0009] | −0.0005 [−0.0029, +0.0032] | −0.0005 [−0.0062, +0.0042] |
| vm (MLP 4898ch) | 2,166.7M | +0.0112 [−0.0003, +0.0245] | +0.0149 [+0.0052, +0.0267]\* | +0.0055 [−0.0036, +0.0295] |
| dual (둘 다) | 2,657.5M | +0.0581 [+0.0361, +0.0829]\* | +0.0524\* | +0.0776\* |

| 게이트 | val500 | test500 | OOD-val | 판정 |
|---|---|---|---|---|
| **V4 파라미터 매칭** vq − vm_pm | **+0.0300 [+0.0084, +0.0547]\*** | **+0.0480 [+0.0242, +0.0780]\*** | +0.0323 [−0.0100, +0.0509] | **축 효과가 본질적, 1차 예측과 반대** |
| **V1 비율 매칭** vq − vm | **+0.0177 [+0.0002, +0.0463]\*** | **+0.0284 [+0.0048, +0.0459]\*** | −0.0072 [−0.0394, +0.0276] | 4.4배 적게 지운 Q가 더 비쌈(2/3 세트) |
| V2 가산성 dual − (vq+vm) | +0.0040 [−0.0079, +0.0122] | +0.0067 [−0.0089, +0.0172] | +0.0215 [−0.0144, +0.0563] | 가산적 |
| 아키텍처 확인 d(dual_bw) − d(dual_ada) | −0.0000 [−0.0012, +0.0012] | — | — | Ada/Blackwell 차이 없음 |

**V5 순위: DISAGREE.** 1차 질량은 vq가 **가장 싸다**고 예측하는데(Σ I_traj: vm 18.1 >
vm_pm 3.16 > vq 1.79) 실측은 vq가 **가장 비싸다**. Stage 1이 스케일 문제를 배제했으므로,
남는 설명은 S2가 Q축에서만 직접 관측한 **층 간 상호작용**이다.

CoC 채널(관찰): 무압축과 같은 CoC 비율 vq 0.570 / vm 0.728 / vm_pm 0.876, 퇴화율
baseline 0.008 대비 vq 0.020 / vm 0.002 / vm_pm 0.006.

### 결론

1. **비교 가능하다** — 두 축의 1차 항은 같은 손실 단위이고, 예측→실측 환산 기울기도 같다
   (β 비 1.066 [0.890, 1.193]). 단 raw 점수에서만이고 znorm/dual에서는 불가능하다.
2. **그러나 1차 질량으로 축 예산을 정하면 안 된다** — VLM에서 예측이 정확히 뒤집힌다.
   실패 원인은 스케일이 아니라 1차 항에 없는 층 간 상호작용이고, 그것은 Q축에만 있다.
3. **두 타워 모두 비용은 Q head에 몰린다** — expert는 완전히(both25의 비용 전부), VLM은
   지배적으로(같은 파라미터에서 Q가 +0.030~0.048 더 비쌈).
4. **실무**: VLM MLP 1109ch/층(490.6M, 전체 4.4%)은 세 세트 모두 무압축과 구별되지 않는다.
   MLP 4898ch/층(2.17B, 19.6%)도 +0.011~0.015에 그치는데, 여기에 head 13개를 더한
   `dual_u40_v2`는 +0.052~0.078이다. 다음 후보는 **예산을 MLP로 몰고 Q를 아끼는 config**다.

미검증: 폐루프, VLM에서의 probe(1차 근사 기울기), 다른 기준(magnitude/J-lens)에서의 축 비대칭.

## 후속: 분포 분석 추가 (2026-08-31, 사용자 요청)

§3의 축 간 비교가 중앙값 요약 하나에 얹혀 있었고, 중앙값(555×)과 평균(7.4×)이 expert에서
75배 어긋난다는 점을 사용자가 지적했다. 새 실험 없이 기존 importance 배열만으로 분포 분석을
추가했다 (`axis_distribution()` + `plot_distribution()`, 보고서 §3.1).

**편중은 한 축에만 있다** (층 내 유닛 기준):

| 축 | 유닛/층 | 평균/중앙값 | Gini | 상위 1% 질량 | 하위 50% 질량 |
|---|---:|---:|---:|---:|---:|
| expert Q | 16 | ×1.5 | 0.372 | — | 24.96% |
| **expert MLP** | 8256 | **×115.5** | **0.972** | **71.3%** | 0.44% |
| VLM Q | 32 | ×2.3 | 0.434 | — | 21.13% |
| VLM MLP | 12288 | ×1.3 | 0.369 | 13.8% | 27.39% |

expert MLP만 극단이고 나머지 셋은 Gini 0.34–0.44로 비슷하다. 그래서 요약 방식에 흔들리는 것도
expert뿐이다(555× vs 7.4×). VLM은 0.35× vs 0.61×로 차이가 작다.

**인용할 숫자는 선택 집합의 부분합이다.** 같은 파라미터에서: expert 4 head(75.5M)가
341 ch(75.4M)의 **1,202배**, VLM 13 head(490.7M)가 1109 ch(490.6M)의 **0.57배**
(= MLP가 1.76배 비쌈). 중앙값·평균·부분합 셋 다 부호는 같고 크기만 다르다.

**부수 발견 — dual은 traj 기준 최적이 아니다.** dual이 고른 집합 vs 그 축에서 `I_traj`가
가장 싼 집합: vq 1.785 vs 1.636 (+9.1%), vm_pm 3.160 vs 2.876 (+9.9%), vm 18.140 vs
17.067 (+6.3%). CoC를 함께 지키는 대가가 trajectory 질량 6–10%다. 두 표가 서로 다른 선택
규칙을 쓴다는 점을 보고서 §3.1 주석에 명시했다.

플롯 `axis_distribution.png` 3패널: 편중도 곡선 / 파라미터 예산 대 절대 질량(양축 로그,
같은 x에서 위쪽 곡선이 비싼 축) / 스케일 제거한 분포 모양.
