...
## 폐루프 (2026-08-23) — alpasim public_2601, 150 씬 × 2 rollouts, 드라이버 Ada 4–7

`launch_alpasim_tyr.sh`(preflight 4항목 통과: 동일 씬 150/150, 예산 2,657,452,032, state 적재,
드라이버 하드링크) → 4샤드 → 머지 → `analyze_alpasim.py`. arm: `slim_tyr_u40_r`.

| config | scene score | pass% | at-fault | off-road | progress | CoC degen |
|---|---|---|---|---|---|---|
| baseline | 0.750 | 89.0 | 0.040 | 0.070 | 0.747 | 0.006 |
| dual | **0.828** | 91.0 | 0.023 | 0.067 | 0.828 | 0.027 |
| Týr (재구성+탐색) | 0.786 | 88.3 | 0.030 | **0.087** | 0.794 | **0.059** |

씬 페어드 (bootstrap 평균 CI 주 판정 / Wilcoxon 보조):
- dual − baseline **+0.0787 [+0.0355, +0.1234]\*** p=0.0001 (W/L/T 56/20/74)
- Týr − baseline +0.0368 [−0.0099, +0.0835] p=0.0072 (58/23/69) — CI 미확정
- **Týr − dual −0.0419 [−0.0848, −0.0004]\*** p=0.0989 (29/34/87) — dual 우세, 경계
  (N=150 해상도 0.080보다 작은 효과)

관찰: **CoC 붕괴율 부호 반전** — 개루프에서 Týr 0.8% < dual 3.0%였으나 폐루프에서는 5.9% > 2.7%이고,
Týr의 붕괴는 전부 빈 출력(soup 0.000, 길이 63 vs 74). 실패 씬(<0.5) 비율 Týr 18.0% vs dual 12.0%.
해석: 재구성은 AV 프리필 분포에서 출력을 복원하도록 맞춰져 있어 분포 밖(시뮬 렌더)·반복 호출에서
이탈·침묵으로 드러난다. 최종 위치 불변(개루프 동급 · 폐루프 열세 · 언어 하한 이하 · 비용 큼).

주의: shipped 런의 driver 스레드 설정 불일치(dual만 `OMP_NUM_THREADS` 미설정)를 이번 런은 baseline
쪽(8)에 맞췄다 — Týr−baseline은 설정 일치, Týr−dual은 불일치 포함.
보고서: `reports/evaluation/2026-08-23_tyr-closedloop.html`. 미실행: `slim_tyr_uniform_u40_recon`
폐루프(전역 탐색 효과 분리, ~7.5 h).
